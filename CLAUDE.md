...
- Imágenes: `pickImageBackend` en server.js decide el backend — Stable Diffusion local (`sdCheck`/`sdGenerate` en providers.js, API Automatic1111, URL en `config.sdWebuiUrl`) tiene prioridad sobre la nube (OpenAI/xAI/Zhipu). `config.customInstructions` se añade al system solo en la copia enviada al proveedor (nunca al historial).
