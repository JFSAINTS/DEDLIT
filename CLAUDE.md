...
- Fallback: `config.fallbackChain` (lista `proveedor:modelo`). En server.js, `streamWithFallback` prueba los candidatos (elegido + cadena) y salta al siguiente solo si el error es recuperable (`isRecoverableError`: 429/402/503/500/529/420 o textos de cuota; NO 401/403/400/404) y aún no se ha emitido nada; emite un evento SSE `fallback` al avisar del cambio.
- `lib/agent.js` — herramientas del modo agente (list_directory, read_file, search_files, write_file, run_command, fetch_url, open_in_browser) con categorías read/write/command/network que controlan si se pide aprobación al usuario. `systemPrompt` auto-carga archivos de conocimiento del workspace (knowledge.md/AGENTS.md/CLAUDE.md/.cursorrules/.dedlit.md). `expandMentions(text, workspace)` expande menciones `@ruta` al contenido del archivo (con guardas anti path-traversal); server.js la aplica al último mensaje del usuario en chatHandler.
...
- `lib/rag.js` — RAG local: extracción de texto (texto/código, PDF con extractor mínimo de streams Flate+Tj, DOCX descomprimiendo el ZIP a mano), troceado con solape, embeddings vía `providers.embed` (/v1/embeddings de cualquier proveedor openai-kind), coseno en JS. Colecciones en `~/.dedlit/rag`. En el chat, `body.ragId` inyecta los 5 fragmentos más afines como system; el agente tiene `search_docs` (se ejecuta en server.js). Reindexado incremental: `buildIndex` acepta `prev` (la colección anterior) y reutiliza embeddings de archivos con igual mtime+size (guardados en `fileMeta`), procesando solo lo cambiado; `/api/rag/index` con un `id` existente lo activa.
...
- `lib/selfsigned.js` — genera un certificado X.509 autofirmado **sin dependencias** (codificador ASN.1/DER propio + clave RSA de node:crypto) para el HTTPS de acceso LAN. `ensure(hosts)` cachea cert/clave en `~/.dedlit` y regenera si cambian los hosts (IPs de red + localhost como SAN). En server.js, `applyListen` levanta un `httpsServer` opcional en `PORT+1` que reusa el mismo handler (`server.emit('request', …)`) cuando `lanHost && lanHttps`.
- Cliente: resaltado de sintaxis propio (`highlightCode`/`HL_RULES` en app.js, tokenizador por familia de lenguaje, escapa HTML y no colorea dentro de strings/comentarios) y contador de tokens/coste (`chatStats`, heurística ~4 car./token + tabla `PRICING` por millón de tokens; locales = gratis).
