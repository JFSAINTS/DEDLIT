...
const TOOLS = [
  ...
  },
  {
    name: 'search_docs',
    category: 'read',
    description: 'Busca por similitud semántica en las colecciones de documentos que el usuario ha indexado (RAG local: sus PDFs, notas, código…). Devuelve los fragmentos más relevantes con su archivo de origen. Úsalo cuando la pregunta trate sobre los documentos del usuario.',
    parameters: {
      type: 'object',
      properties: {
        query: { type: 'string', description: 'Pregunta o términos a buscar' },
        top_k: { type: 'number', description: 'Número de fragmentos (por defecto 5)' }
      },
      required: ['query']
    }
  },
  ...
  },
  {
    name: 'generate_image',
    category: 'network',
    description: 'Genera una imagen a partir de un texto y se la muestra al usuario en el chat. Usa automáticamente Stable Diffusion local (Automatic1111/Forge, gratis y privado) si está activo, o el proveedor en la nube con key (OpenAI, xAI, Zhipu). Si no hay ningún backend, dile al usuario cómo activarlo (SD WebUI con --api, o una key).',
    parameters: {
      type: 'object',
      properties: {
...
// Carga automática de archivos de contexto del proyecto (como Claude Code o
// Codebuff): si el workspace tiene alguno de estos, su contenido se añade al
// system prompt para que el agente conozca las convenciones del proyecto.
const KNOWLEDGE_FILES = ['knowledge.md', 'AGENTS.md', 'CLAUDE.md', '.cursorrules', '.dedlit.md'];

function readWorkspaceKnowledge(workspace) {
  const out = [];
  for (const name of KNOWLEDGE_FILES) {
    try {
      const p = path.join(workspace, name);
      if (fs.existsSync(p) && fs.statSync(p).isFile()) {
        const text = fs.readFileSync(p, 'utf8').slice(0, 8000);
        if (text.trim()) out.push('=== ' + name + ' ===\n' + text.trim());
      }
...
// Menciones @archivo: expande "@ruta" del texto del usuario al contenido del
// archivo (dentro del workspace). Devuelve un bloque de contexto o ''.
function expandMentions(text, workspace) {
  ...
  while ((m = re.exec(text))) {
    let rel = m[1].replace(/[.,;:)]+$/, ''); // quitar puntuación final
    if (!rel || seen.has(rel)) continue;
    seen.add(rel);
    try {
      const abs = path.resolve(wsRoot, rel);
      if (!abs.startsWith(wsRoot)) continue;           // fuera del workspace: ignorar
    ...
    } catch { /* no existe o ilegible: se deja el @ tal cual */ }
  }
  return blocks.length ? 'Archivos mencionados por el usuario (@):\n\n' + blocks.join('\n\n') : '';
}
...
function systemPrompt(workspace) {
  ...
  const knowledge = readWorkspaceKnowledge(workspace);
  const knowledgeBlock = knowledge
    ? `\n\nContexto del proyecto (archivos de conocimiento del workspace; respeta sus convenciones):\n${knowledge}`
    : '';
...
- Responde siempre en el idioma del usuario.${knowledgeBlock}`;
}

module.exports = { TOOLS, toolCategory, execute, systemPrompt, expandMentions, _test: { htmlToText, globToRegex, searchFiles, readWorkspaceKnowledge, expandMentions } };
