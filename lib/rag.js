...
// Construye o reindexa una colección. Si `prev` es la colección anterior
// (reindexado incremental), reutiliza los embeddings de los archivos cuyo
// tamaño y fecha de modificación no han cambiado, y solo procesa los nuevos
// o modificados. `prev` a null = indexado completo desde cero.
async function buildIndex({ id, name, folder, provider, model, cfg, onProgress, prev }) {
  ensure();
  ...
  // el reindexado solo reutiliza si coinciden proveedor y modelo de embeddings
  const canReuse = prev && prev.provider === provider && prev.model === model;
  const prevMeta = canReuse ? (prev.fileMeta || {}) : {};
  const prevChunksByFile = {};
  if (canReuse) {
    for (const c of prev.chunks || []) (prevChunksByFile[c.file] ??= []).push(c);
  }

  const fileMeta = {};
  const reused = [];        // chunks (con vector) reutilizados sin cambios
  const toEmbed = [];       // chunks nuevos pendientes de embeddings
  let unchanged = 0, changed = 0;

  for (const f of files) {
    const rel = path.relative(folder, f);
    let st; try { st = fs.statSync(f); } catch { continue; }
    fileMeta[rel] = { mtime: st.mtimeMs, size: st.size };
    const pm = prevMeta[rel];
    if (pm && pm.mtime === st.mtimeMs && pm.size === st.size && prevChunksByFile[rel]) {
      reused.push(...prevChunksByFile[rel]); // sin cambios: reutilizar embeddings
      unchanged++;
      continue;
    }
    changed++;
    ...
    if (text) toEmbed.push(...chunkText(text, rel));
    if (reused.length + toEmbed.length >= 8000) break; // límite global
  }
  if (!reused.length && !toEmbed.length) throw new Error('No se pudo extraer texto de ningún documento');

  // embeddings solo de lo nuevo/modificado, por lotes
  const BATCH = 24;
  for (let i = 0; i < toEmbed.length; i += BATCH) {
    ...
    if (onProgress) onProgress(Math.min(i + BATCH, toEmbed.length), toEmbed.length);
  }

  const chunks = [...reused, ...toEmbed];
  const collection = { id, name, folder, provider, model, createdAt: (prev && prev.createdAt) || Date.now(), updatedAt: Date.now(), files: files.length, chunks, fileMeta };
  fs.writeFileSync(fileOf(id), JSON.stringify(collection));
  return { id, name, files: files.length, chunks: chunks.length, unchanged, changed };
}
