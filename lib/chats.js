...
// Caché de metadatos en memoria: evita leer y parsear todos los chats del
// disco en cada listado (se actualiza en save/remove; proceso único)
let metaCache = null;

function metaOf(c) {
  return { id: c.id, title: c.title || 'Sin título', updatedAt: c.updatedAt || 0, count: (c.messages || []).length, projectId: c.projectId || '' };
}
  ...
  if (!metaCache) {
    ...
    for (const f of fs.readdirSync(DIR)) {
      ...
      try {
        ...
        metaCache.set(c.id, metaOf(c));
      } catch { /* archivo corrupto: ignorar */ }
    }
  }
  return [...metaCache.values()].sort((a, b) => b.updatedAt - a.updatedAt);
}
...
function save(chat) {
  ...
  if (metaCache) metaCache.set(chat.id, metaOf(chat));
  return chat.id;
}

function remove(id) {
  try { fs.unlinkSync(fileOf(id)); } catch { /* ya no existe */ }
  if (metaCache) metaCache.delete(id);
}
