...
const REGISTRY = {
  ollama:    { name: 'Ollama (local)',        kind: 'openai',    base: 'http://localhost:11434/v1', needsKey: false, local: true },
  lmstudio:  { name: 'LM Studio (local)',     kind: 'openai',    base: 'http://localhost:1234/v1',  needsKey: false, local: true },
...
function baseUrl(providerId, cfg) {
  return (cfg.baseUrls[providerId] || REGISTRY[providerId].base).replace(/\/+$/, '');
}
